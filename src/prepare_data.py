...
import pickle, cPickle, csv, gzip, h5py, yaml
...
def pack_features_to_hdf5(fe_dir, out_path, yaml_dir=None):
    ...
    if not yaml_dir==None:    # Pack from csv file (training & testing from dev. data)
        lis=[]
        for file in sorted(os.listdir(yaml_dir)):
            if file.endswith('yaml'):
                lis += read_yaml(os.path.join(yaml_dir,file))
        cnt = 0
        for li in lis:
            na=li['mixture_audio_filename']
            x=cPickle.load(open(os.path.join(fe_dir,na.replace('wav','p')), 'rb'))
            x = pad_trunc_seq(x, max_len)
            y=np.zeros((x.shape[0],cfg.num_classes))
        ...
        for fe_na in sorted(os.listdir(fe_dir)):
